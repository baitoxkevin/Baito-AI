...
export default function TestCandidateUpdatePage() {
  ...
  useEffect(() => {
    if (token) {
      ...
      // For testing without token - set default test data
      setLoading(false);
      setIsEditMode(true);
      setFormData({
        full_name: 'Test User',
  ...
  const fetchCandidateData = async () => {
    ...
    try {
      ...
      setCandidate(linkData.candidates);
      setIsEditMode(true);
      
      // Initialize form with candidate data
      const candidateData = linkData.candidates;
      setFormData({
        ...
        languages_spoken: candidateData.languages_spoken || 'Other',
        passport_number: candidateData.passport_number || '',
        custom_fields: candidateData.custom_fields || {},
        pdpa_consent: candidateData.pdpa_consent || false
      });
      
      // Check if IC/Passport was previously edited
      if (candidateData.ic_number || candidateData.passport_number) {
  ...
  const confirmSaveChanges = async () => {
    ...
    try {
      ...
      if (halfBodyPhoto && halfBodyPhoto.startsWith('data:')) {
        ...
        const halfBodyName = `${candidate?.id || 'temp'}-halfbody-${Date.now()}.jpg`;
        const { data: halfBodyUpload, error: halfBodyError } = await supabase.storage
          .from('candidate-photos')
          .upload(halfBodyName, halfBodyFile, { upsert: true });
          
        if (halfBodyError) throw halfBodyError;
        halfBodyPhotoUrl = halfBodyUpload.path;
      }
      ...
      const { error: updateError } = await supabase
        .from('candidates')
        .update(updateData)
        .eq('id', candidate?.id || formData.id);
      ...
      await supabase
        ...
        .insert({
          candidate_id: candidate?.id || formData.id,
          action_type: 'profile_update',
          action_details: {
            updated_fields: Object.keys(updateData),
            update_source: 'self_service'
          },
          ip_address: null, // Would need to get this from request
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 via-cyan-50 to-blue-100">
          <Card className="max-w-md mx-auto p-8">
            <div className="flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-sm text-gray-600">Loading candidate information...</p>
            </div>
          </Card>
        </div>
      </PublicPageWrapper>
                ...
                <div className="w-full flex flex-col flex-1">
                  <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 flex flex-col">
                    <MagicCard className="relative w-full overflow-visible rounded-lg mb-2">
                      ...
                      <TabsTrigger value="personal" className="text-[10px] px-1 py-1.5 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-blue-600 transition-all">
                        <UserPlus className="h-3 w-3 mr-0.5" />
                        <span>Info</span>
                      </TabsTrigger>
                      <TabsTrigger value="contact" className="text-[10px] px-1 py-1.5 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-blue-600 transition-all">
                        <MapPin className="h-3 w-3 mr-0.5" />
                        <span>Addr</span>
                      </TabsTrigger>
                      <TabsTrigger value="photos" className="text-[10px] px-1 py-1.5 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-blue-600 transition-all">
                        <Camera className="h-3 w-3 mr-0.5" />
                        <span>Pics</span>
                      </TabsTrigger>
                      <TabsTrigger value="experience" className="text-[10px] px-1 py-1.5 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-blue-600 transition-all">
                        <Briefcase className="h-3 w-3 mr-0.5" />
                        Skills
                      </TabsTrigger>
                      <TabsTrigger value="banking" className="text-[10px] px-1 py-1.5 data-[state=active]:bg-white data-[state=active]:shadow-sm data-[state=active]:text-blue-600 transition-all">
