...
function CalendarPicker({
  ...
  // Use a wider date range to ensure all dates are selectable
  const startDate = new Date(1950, 0);
  const endDate = new Date(2050, 11);
  
  // Stop event propagation on all clicks within the calendar
  ...
  return (
    <div className={className} onClick={handleCalendarClick}>
      <Calendar
        mode="single"
        selected={selected}
        onSelect={onSelect}
                ...
                <div key={year.getFullYear()} ref={isCurrentYear ? currentYearRef : undefined}>
                  <CollapsibleYear title={year.getFullYear().toString()} open={isCurrentYear}>
                    <div className="grid grid-cols-3 gap-2">
                      {months.map((month) => {
                        // Temporarily disable the date validation to allow all dates
                        const isDisabled = false; // was: isBefore(month, startDate) || isAfter(month, endDate);
