...
export default function ReceiptScanner({ onReceiptScanned, userId }: ReceiptScannerProps) {
  ...
  async function uploadReceiptImage(file: File): Promise<string> {
    try {
      ...
      if (!validateImage(file)) {
        throw new Error('Receipt must be a JPEG, PNG, HEIC image, or PDF');
      }
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Process with simulated OCR (replacing the Gemini API call)
      const receiptData = await simulateOcrProcessing(imageUrl, userId);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to process the receipt';
  ...
  const applyReceiptData = () => {
    if (scannedReceipt) {
      ...
      toast({
        ...
        description: `Amount $${scannedReceipt.amount.toFixed(2)} from ${scannedReceipt.vendor} has been added`,
        variant: "success"
      });
      ...
      <CardFooter className="flex justify-between border-t p-4 pt-6">
        {scannedReceipt ? (
          ...
          <div className="flex justify-between w-full">
            ...
            <div className="text-xs text-muted-foreground bg-yellow-50 dark:bg-yellow-900/20 text-yellow-800 dark:text-yellow-300 rounded-md px-2 py-1">
              Using simulated OCR (Gemini API temporarily disabled)
            </div>
          </div>
        )}
      </CardFooter>
