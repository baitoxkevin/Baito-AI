...
export function ECPExportDialog({
  ...
  const paymentsWithoutIC = batch.payments.filter(p => !p.ic_number).length;
  const paymentsWithoutBankDetails = batch.payments.filter(p => !p.bank_account_number).length;
  const paymentsWithPassport = batch.payments.filter(p =>
  ...
  const handleExport = async () => {
    try {
      ...
      const batchRef = `ECP-${batch.batch_reference}-${timestamp}`;
      ...
      // Generate ECP file
      const blob = await generateECPExport(batch, options);
          ...
          <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <span className="ml-2 font-medium">{batch.batch_reference}</span>
              </div>
              <div>
                <span className="text-gray-600 dark:text-gray-400">Total Payments:</span>
                <span className="ml-2 font-medium">{batch.payments.length}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="ml-2 font-medium">{batch.project_name || batch.project_id}</span>
              </div>
            </div>
          </div>
