...
// 扩展 Project 类型以包含 hasApplied 属性
interface ProjectWithApplication extends Project {
  hasApplied?: boolean;
}
...
export function CandidateProjectApplications({ candidateId, candidateName }: CandidateProjectApplicationsProps) {
  const [openProjects, setOpenProjects] = useState<ProjectWithApplication[]>([]);
  const [myApplications, setMyApplications] = useState<Application[]>([]);
  const [loading, setLoading] = useState(true);
  const [applying, setApplying] = useState(false);
  const [selectedProject, setSelectedProject] = useState<ProjectWithApplication | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // 为每个项目添加 hasApplied 标记
      const projectsWithApplicationStatus: ProjectWithApplication[] = projects.map(project => ({
        ...project,
        hasApplied: appliedProjectIds.has(project.id)
      }));

      setOpenProjects(projectsWithApplicationStatus);
  ...
  const getApplicationStatus = (application: Application): string => {
    return application.working_dates_with_salary?.status || 'pending';
  };

  // 过滤未申请的项目
  const availableProjects = openProjects.filter(project => !project.hasApplied);
    ...
    <div className="w-full space-y-4">
      ...
      <div className="flex space-x-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
        <button
          ...
          className={`flex-1 px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'open'
        ...
        <button
          ...
          className={`flex-1 px-4 py-2 rounded-md text-sm font-medium transition-colors ${activeTab === 'applications'
            ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm'
            : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
            }`}
        >
          ...
          {availableProjects.length === 0 && (
            <div className="col-span-full text-center py-8 text-slate-500">
              {openProjects.length === 0
                ...
                : "You have already applied to all available projects."
              }
            </div>
          )}
        </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div className="space-y-2">
                    <div className="flex items-center text-slate-600 dark:text-slate-400">
                      <Calendar className="w-4 h-4 mr-2" />
                      {application.projects?.start_date && format(new Date(application.projects.start_date), 'dd MMM yyyy')}
                    </div>
                    <div className="flex items-center text-slate-600 dark:text-slate-400">
