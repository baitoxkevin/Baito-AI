-- Payment System for Gig Workers (FIXED)
...
DROP TABLE IF EXISTS salary_configurations CASCADE;
...
BEGIN
  ...
  SELECT
    COALESCE(SUM(EXTRACT(EPOCH FROM (check_out_time - check_in_time)) / 3600), 0)
  INTO v_hours_worked
  FROM attendance
