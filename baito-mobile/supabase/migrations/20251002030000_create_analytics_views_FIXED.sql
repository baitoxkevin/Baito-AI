-- Analytics Views and Functions for Admin Dashboard (FIXED)
...
SELECT
  ...
  c.full_name,
  c.email,
  -- Removed c.phone as it doesn't exist
  ...
  COALESCE((SELECT SUM(points_awarded) FROM achievements WHERE candidate_id = c.id), 0) as total_points,
  CASE
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'total_shifts_completed', (SELECT COUNT(*) FROM attendance WHERE status = 'checked_in' AND DATE(check_in_time) BETWEEN start_date AND end_date),
    'total_hours_worked', (
    ...
    'total_expenses', (SELECT COALESCE(SUM(amount), 0) FROM expense_claims WHERE status = 'approved' AND DATE(created_at) BETWEEN start_date AND end_date),
