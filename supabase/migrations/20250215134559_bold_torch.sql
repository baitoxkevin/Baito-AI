...
CREATE TABLE candidates (
  ...
  gender gender_type,
  date_of_birth date,
  age integer, -- Changed from generated column to regular column
  nationality text,
...
CREATE OR REPLACE FUNCTION update_candidate_age()
RETURNS TRIGGER AS $$
BEGIN
...
-- Create trigger to update age
CREATE TRIGGER update_candidate_age_trigger
  BEFORE INSERT OR UPDATE OF date_of_birth ON candidates
  FOR EACH ROW
  EXECUTE FUNCTION update_candidate_age();
