...
-- Add supervisor_id column to crew_assignments if it doesn't exist
ALTER TABLE crew_assignments
  ADD COLUMN IF NOT EXISTS supervisor_id uuid REFERENCES candidates(id);

-- Create index for supervisor relationships
CREATE INDEX IF NOT EXISTS idx_crew_assignments_supervisor 
  ON crew_assignments(supervisor_id);

-- Create view for crew assignments with supervisor info
CREATE VIEW crew_assignments_with_supervisors AS
SELECT 
  ca.id,
  ca.project_id,
  ca.position_number,
